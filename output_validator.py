...
def validate_output(input_file, output_file, params=[]):
    # print('\n Processing Validate_output', input_file)

    input_data = utils.read_file(input_file)
    output_data = utils.read_file(output_file)

    input_message, input_error = input_validator.tests(input_file)
    # print(" input_error = " + str(input_error))
    cost, output_message = tests(input_data, output_data, params=params)
...
def tests(input_data, output_data, params=[]):
    number_of_locations, number_of_houses, list_of_locations, list_of_houses, starting_location, adjacency_matrix = data_parser(
        input_data)
    try:
    ...
    message = ''
    cost = -1
    car_cycle_in_index = output_data[0]
    car_cycle_in_name = []
    for node in car_cycle_in_index:
        car_cycle_in_name.append(list_of_locations.index(node))
    print(" *** Final Results")
    print(" * Car_cycle         (nodes in index): " + str(car_cycle_in_name))
    print(" * Car_cycle            (string name): \n" + str(car_cycle_in_index))  # strings names as written in .out file):
    print(" * Car_cycle  (number of edges taken): " + str(len(car_cycle_in_index)) + "\n")
    ...
    for i in range(num_dropoffs):
        ...
        if dropoff[0] not in list_of_locations:
            ...
            cost = 'infinite'
        if dropoff[0] not in car_cycle_in_index:
            print("dropoff[0] : " + dropoff[0])
    ...
    if (car_cycle_in_index[0] != starting_location):
        message += "Your car must start at the specified starting location.\n"
        cost = 'infinite'

    car_cycle_in_index = convert_locations_to_indices(car_cycle_in_index, list_of_locations)

    if (car_cycle_in_index[0] != car_cycle_in_index[-1]):
        message += "Your car must start and end at the same location.\n"
        cost = 'infinite'

    if cost != 'infinite':
        cost, solution_message = cost_of_solution(G, car_cycle_in_index, dropoffs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--all', action='store_true',
                        help='If specified, the output validator is run on all files in the output directory. Else, it is run on just the given output file')
